...
import { ArrowRight, Sparkles, Mail, Lock, CheckCircle2 } from 'lucide-react';
...
export default function LoginPage() {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 403) {
          // Email not verified
          setStep('otp');
          setMessage('Email not verified. A verification code has been sent to your email.');
          return;
        }
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Successful OTP -> Redirect to dashboard with token
      window.location.href = `${dashboardUrl}?token=${data.token}`;
    } catch (err: any) {
      setError(err.message);
    } finally {
      ...
      <div className="w-full max-w-md bg-secondary/30 backdrop-blur-xl border border-border/50 p-8 md:p-10 rounded-3xl shadow-2xl relative z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-secondary-foreground text-center mt-2 font-medium">
            {step === 'login' 
              ? 'Access your smart job automation dashboard' 
