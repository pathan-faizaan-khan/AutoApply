 ...
 *  • Per-request timeout is handled by a manual setTimeout + AbortController
 *    (avoids AbortSignal.any / AbortSignal.timeout TS lib-target issues).
...
async function wakeupService(
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      // Manual combined controller — avoids AbortSignal.any / .timeout TS errors
      ...
      // Wait before retrying, bail early if parent is aborted
...
export function useBackendWakeup(configs: WakeupConfig[]) {
  // Stable ref — lets the effect closure always read the latest configs
  // without adding configs to the dependency array.
  ...
  const [states, setStates] = useState<Record<string, WakeupState>>(() =>
    Object.fromEntries(
      configs.map((c) => [
        c.name,
        { status: "idle" as WakeupStatus, attempt: 0, error: null },
      ])
    )
  );
  ...
  useEffect(() => {
    ...
    configsRef.current.forEach((config) => {
      wakeupService(
        config,
        (patch) => {
          // `as WakeupState` — spreading Partial<WakeupState> onto WakeupState
          // widens the inferred type; the cast restores Record<string, WakeupState>
          setStates((prev) => ({
            ...prev,
  ...
  }, []); // Runs once on mount — configsRef keeps the closure current
